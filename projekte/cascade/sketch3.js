...
function drawSmartwatchFrame() {
  rectMode(CENTER);

  // äußerer quadratischer Smartwatch-Rahmen
  noStroke();
  fill(COLOR_DARK_GRAY);
  rect(width / 2, height / 2, width - 8, height - 8, 42);

  // schwarze Displayfläche
  fill(COLOR_BLACK);
  rect(width / 2, height / 2, width - 36, height - 36, 28);

  // Kein zusätzlicher kühlgrauer Rand
}
...
function createSmartwatchBounds() {
  ...
  // Boden, auf dem Kugeln landen und langsam verschwinden
...
function createCircleParticle(indexInSpawn, totalInSpawn) {
  ...
  */
  ...
  let x = CENTER_X + offset + random(-6, 6);
  let y = 35;
